...
function setup() {
  ...
  <input type="radio"> brightness </form></body>');
  radio1 = createRadio();
  radio1.option(" ", 1);
  radio1.option(" ", 2);
  radio1.option(" ", 3);
...
function displayTrial() {
  ...
  if ( feedback === true ) { // training
    fill(object_color[trainset_ID[train_trial_counter-1]]); 
    //text("object: "+str(trainset_ID[train_trial_counter-1]),30,30);
  }
  if ( feedback === false ) { // testing
    //xxx
    fill(object_color[testset_ID[test_trial_counter-1]]);
    //text("object: "+str(testset_ID[test_trial_counter-1]),30,30);
    //fill(object_color[trainset_ID[test_trial_counter-1]]);
    //text("object: "+str(testset_ID[train_trial_counter-1]),30,30);
    push(); fill(0);
    //text(testset_ID,30,30);
    //text(test_trial_counter,30,60);
    //text(testset_ID[train_trial_counter-1],30,90);
    //text(object_color[testset_ID[test_trial_counter-1]],30,120);
    pop();
  }
...
function displayFeedback() {
  ...
  push(); 
  fill(object_color[trainset_ID[train_trial_counter-1]]); // training only, because we're in displayFeedbck() now
  ellipse(width/2,height/2,180,180); 
  pop();
...
function displayInstructions1() {
  ...
  They are two types of shells.  There is only one way to tell these shells apart - it's by how bright they are.",leftmargin,bodymargin,750,600);
